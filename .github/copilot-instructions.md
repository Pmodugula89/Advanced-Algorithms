...
- `.github/workflows/*.yml` — CI runs `flake8` and `pytest`.
...
- Run the pipeline against the example fixture (no plotting):
...
Developer workflows (lint/test)
- Lint locally: `pip install flake8` then `flake8 .` (CI uses `--max-line-length=127` and `--max-complexity=10`).
- Tests: CI runs `pytest`. The repo includes mock-based fast tests so tests can run even in minimal environments; full tests that exercise pandas will run when dependencies are installed (CI).
...
- The code mixes CLI-style printing and plotting. For automated runs (CI, headless servers), use `--no-plot`.
- `src/Main.py` now supports `--data-path`, `--no-plot` and `--output`.
...
- Do not rely on interactive plotting in CI.
